...
export function BmrAssignDialog({ token, instance, onClose, onSaved }: {
  ...
  useEffect(() => {
    listBmrOperators(token).then((r) => setOperators(r.operators)).catch((e) => setError(e instanceof Error ? e.message : t('common.error')))
  }, [token, t])
  ...
  async function save() {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : t('bmrAssign.saveFailed')) }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-5 py-3">
          ...
          <div>
            <div className="text-[15px] font-bold text-slate-900">{t('bmrAssign.title')}</div>
            <div className="text-[12px] text-slate-500">{t('bmrAssign.subtitle', { batch: instance.batch_no ?? '' })}</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {stages.length === 0 && <div className="text-[13px] text-slate-400">{t('bmrAssign.noStages')}</div>}
            ...
            {stages.map((st) => (
              <div key={st.stage} className="rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-1 gap-2 p-3 md:grid-cols-2">
                  ...
                  {operators.length === 0 && <span className="text-[12px] text-slate-400">{t('bmrAssign.noOperators')}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex items-center gap-2 border-t border-slate-200 px-5 py-3">
          <span className="text-[11px] text-slate-400">{t('bmrAssign.footerHint')}</span>
          ...
          <button disabled={busy || closed} onClick={() => void save()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-blue-600 px-4 text-[13px] font-semibold text-white disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} {t('bmrAssign.saveAssignments')}</button>
        </div>
      </div>
    </div>
  )
