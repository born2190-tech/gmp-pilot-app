...
function roleLabel(role: SignRole, t: Translate): string {
  return t(`weighing.role.${role}` as Parameters<Translate>[0])
}
function statusLabel(status: string, t: Translate): string {
  const known = ['draft', 'active', 'completed']
  return known.includes(status) ? t(`weighing.status.${status}` as Parameters<Translate>[0]) : status
}

function fmtTime(iso: string | undefined, locale: string): string {
  if (!iso) return ''
  const d = new Date(iso)
  return isNaN(d.getTime()) ? '' : d.toLocaleString(locale, { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })
}
...
export function WeighingCampaignPage({ token, user }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : t('weighing.loadFailed')) }
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[22px] font-bold tracking-tight text-slate-900">{t('weighing.title')}</h1>
          <p className="text-[13px] text-slate-500">{t('weighing.subtitle')}</p>
        </div>
        <button onClick={() => setCreating(true)} className="ml-auto inline-flex h-9 items-center gap-2 rounded-lg bg-amber-600 px-3 text-[13px] font-semibold text-white hover:bg-amber-700"><Plus size={16} /> {t('weighing.newCampaign')}</button>
      </div>
      {error && <div className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[13px] text-rose-700">{error}</div>}
      {loading ? (
        <div className="flex items-center gap-2 py-16 text-slate-400"><Loader2 size={18} className="animate-spin" /> {t('common.loading')}</div>
      ) : list.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-400">{t('weighing.emptyList')}</div>
      ) : (
          ...
          {list.map((c) => (
            <button key={c.id} onClick={() => setOpenId(c.id)} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left hover:border-amber-300 hover:bg-amber-50/40">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mono text-[11px] text-slate-400">{c.campaign_date} · {c.room || '—'} · {t('weighing.seriesCount', { n: c.series_count })}</div>
              </div>
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600">{statusLabel(c.status, t)}</span>
...
function CreateDialog({ token, onClose, onCreated }: { token: string; onClose: () => void; onCreated: (id: string) => void }) {
  ...
  const [room, setRoom] = useState(() => t('weighing.roomDefault'))
  ...
  async function submit() {
    ...
    try {
      ...
      onCreated(c.id)
    } catch (e) { setError(e instanceof Error ? e.message : t('weighing.createFailed')) }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-5 py-3">
          ...
          <div className="text-[15px] font-bold text-slate-900">{t('weighing.createTitle')}</div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <label className="text-[12px] text-slate-500">{t('weighing.campaignDate')}
            ...
            <label className="text-[12px] text-slate-500">{t('weighing.room')}
          ...
          <div className="mb-1 text-[12px] font-semibold text-slate-600">{t('weighing.daySeries', { n: picked.size })}</div>
          <div className="space-y-1.5">
            ...
            {batches.length === 0 && <div className="text-[12px] text-slate-400">{t('weighing.noBatches')}</div>}
          </div>
        </div>
        <div className="flex items-center gap-2 border-t border-slate-200 px-5 py-3">
          <button onClick={onClose} className="ml-auto inline-flex h-10 items-center rounded-lg border border-slate-300 bg-white px-4 text-[13px] font-medium text-slate-600">{t('common.cancel')}</button>
          <button disabled={busy} onClick={() => void submit()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-amber-600 px-4 text-[13px] font-semibold text-white disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} {t('common.create')}</button>
        </div>
      </div>
    </div>
  )
...
function CampaignDetail({ token, user, id, onBack }: { token: string; user: CurrentUser | null; id: string; onBack: () => void }) {
  ...
  if (!camp) return <div className="flex items-center gap-2 p-10 text-slate-400"><Loader2 size={18} className="animate-spin" /> {t('common.loading')}</div>
  ...
  const setNet = async (ingredient_key: string, batch_id: string, net: number | null) => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : t('weighing.saveNetFailed')) }
    finally { setBusy(false) }
  }
  const setLot = async (ingredient_key: string, lot_no: string) => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : t('weighing.saveLotFailed')) }
    finally { setBusy(false) }
  }
  ...
  return (
    <div className="mx-auto max-w-[1600px] p-4">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <button onClick={onBack} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-slate-300 bg-white px-2.5 text-[12px] font-medium text-slate-600 hover:bg-slate-50"><ArrowRight size={14} className="rotate-180" /> {t('weighing.backToCampaigns')}</button>
        ...
        <div>
          ...
          <div className="mono text-[11px] text-slate-400">{camp.campaign_date} · {camp.room || '—'} · {t('weighing.seriesCount', { n: camp.batches.length })}</div>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700">{statusLabel(camp.status, t)}</span>
          {canManage && !closed && (
            <button disabled={busy} onClick={async () => { setBusy(true); try { setCamp(await setWeighingStatus(token, id, 'completed')) } finally { setBusy(false) } }} className="inline-flex h-9 items-center gap-2 rounded-lg bg-slate-900 px-3 text-[13px] font-semibold text-white hover:bg-slate-800 disabled:opacity-50"><ShieldCheck size={15} /> {t('weighing.closeCampaign')}</button>
          )}
        </div>
      </div>
      {error && <div className="mb-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[13px] text-rose-700">{error}</div>}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        {camp.batches.map((b) => (
          <span key={b.batch_id} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[12px]">
            ...
            {!b.bmr_instance_id && <span className="rounded bg-rose-50 px-1 text-[9px] text-rose-600">{t('weighing.noBmr')}</span>}
            {canManage && !closed && (
        ...
        <table className="w-full min-w-[720px]">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 text-left text-[10.5px] font-semibold uppercase tracking-wide text-slate-500">
              <th className="px-3 py-2">{t('weighing.colIngredient')}</th>
              <th className="px-3 py-2">{t('weighing.colLotFefo')}</th>
              {camp.batches.map((b) => <th key={b.batch_id} className="px-3 py-2 text-center">{t('weighing.colSeries', { no: b.batch_no })}</th>)}
            </tr>
          </thead>
          <tbody>
            {camp.ledger.length === 0 && (
              <tr><td colSpan={2 + camp.batches.length} className="px-3 py-8 text-center text-[12.5px] text-slate-400">{t('weighing.emptyLedger')}</td></tr>
            )}
            {camp.ledger.map((row) => (
              <tr key={row.key} className="border-b border-slate-100 align-top">
                ...
                <td className="px-3 py-2.5">
                  <input defaultValue={row.lot_no || ''} disabled={closed} onBlur={(e) => { if ((e.target.value || '') !== (row.lot_no || '')) void setLot(row.key, e.target.value) }}
                    placeholder={t('weighing.lotNoPlaceholder')} className="h-9 w-32 rounded-md border border-slate-300 px-2 text-[12.5px] outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100 disabled:bg-slate-50" />
                </td>
                {batchOrder.map((bid) => {
      ...
      {sign && (
        <SignDialog role={sign.role} label={sign.label} busy={busy}
          ...
          onConfirm={async (username, password) => {
            setBusy(true); setError(null)
            try {
              const updated = await signWeighingCell(token, id, { ingredient_key: sign.ingredient_key, batch_id: sign.batch_id, role: sign.role, username, password, meaning: t('weighing.signMeaning', { role: roleLabel(sign.role, t) }), reason: sign.label })
              setCamp(updated); setSign(null)
            } catch (e) { setError(e instanceof Error ? e.message : t('weighing.signRejected')) }
            finally { setBusy(false) }
          }} />
      )}
    </div>
...
function Cell({ cell, closed, canManage, planned, onNet, onSign }: {
  ...
  const sigChip = (role: SignRole, sig: { by: string; at: string } | null | undefined, enabled: boolean) => {
    if (sig) return (
      <span className="inline-flex items-center gap-1 rounded-md border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[10px] text-emerald-700"><Check size={10} /> {roleLabel(role, t)}: {sig.by.split(' ')[0]}</span>
    )
    if (!enabled) return <span className="inline-flex items-center gap-1 rounded-md border border-dashed border-slate-300 px-1.5 py-0.5 text-[10px] text-slate-400"><Lock size={10} /> {roleLabel(role, t)}</span>
    return <button onClick={() => onSign(role)} className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-semibold text-white ${ROLE_TONE[role]}`}><Pen size={10} /> {roleLabel(role, t)}</button>
  }
  return (
    <div className="flex flex-col items-center gap-1.5">
      {planned && <span className="mono text-[10px] text-slate-400">{t('weighing.planLabel', { n: planned })}</span>}
      <input type="number" defaultValue={cell.net ?? ''} disabled={closed || whSigned || !canManage}
        ...
        placeholder={t('weighing.netPlaceholder')} className="h-9 w-24 rounded-md border border-slate-300 px-2 text-center text-[12.5px] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50 disabled:text-slate-500" />
      ...
      {qaSigned && <span className="mono text-[9px] text-slate-400">{fmtTime(cell.qa?.at, locale)}</span>}
    </div>
  )
}
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-4">
      <div className="mx-auto max-w-2xl overflow-hidden rounded-xl border bg-white shadow-2xl">
        <div className={`flex items-center gap-2 px-4 py-2 text-white ${ROLE_TONE[role].split(' ')[0]}`}>
          <Pen size={15} /><span className="text-[12.5px] font-semibold">{t('weighing.signLabel', { role: roleLabel(role, t) })} · {label}</span>
        </div>
        <div className="flex flex-wrap items-center gap-3 px-4 py-3">
          <div className="flex-1 min-w-[160px] text-[12px] text-slate-600">{t('weighing.signHint')}</div>
          <input autoFocus value={u} onChange={(e) => setU(e.target.value)} placeholder={t('weighing.signerLogin')} autoComplete="off" className="h-11 w-44 rounded-lg border border-slate-300 px-3 text-[14px] outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100" />
          <input type="password" value={p} onChange={(e) => setP(e.target.value)} placeholder={t('weighing.passwordPin')} autoComplete="off" onKeyDown={(e) => { if (e.key === 'Enter' && ready) onConfirm(u.trim(), p) }} className="h-11 w-44 rounded-lg border border-slate-300 px-3 text-[14px] outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100" />
          <button disabled={busy || !ready} onClick={() => onConfirm(u.trim(), p)} className="inline-flex h-11 items-center gap-2 rounded-lg bg-amber-600 px-4 text-[13px] font-semibold text-white disabled:opacity-50">{busy ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} {t('common.confirm')}</button>
